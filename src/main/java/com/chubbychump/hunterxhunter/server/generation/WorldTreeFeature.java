...
public class WorldTreeFeature extends Feature<BaseWorldTreeFeatureConfig> {
    public WorldTreeFeature(Codec<BaseWorldTreeFeatureConfig> p_i231999_1_) {
        super(p_i231999_1_);
    }
     ...
     */
    private boolean place(IWorldGenerationReader generationReader, Random rand, BlockPos positionIn, Set<BlockPos> p_225557_4_, Set<BlockPos> p_225557_5_, MutableBoundingBox boundingBoxIn, BaseWorldTreeFeatureConfig configIn) {
        WorldTreeTrunkPlacer yeet = new WorldTreeTrunkPlacer(31, 23, 23);
        BaseTreeFeatureConfig bruh = new BaseTreeFeatureConfig.Builder(
        ...
        if (blockpos.getY() >= 1 && blockpos.getY() + i + 1 <= 256) {
            if (!isDirtOrFarmlandAt(generationReader, blockpos.down())) {
                ...
                if (l1 >= i || optionalint.isPresent() && l1 >= optionalint.getAsInt()) {
                    List<FoliagePlacer.Foliage> list = yeet.func_230382_a_(generationReader, rand, l1, blockpos, p_225557_4_, boundingBoxIn, bruh);
                    list.forEach((p_236407_8_) -> {
                        configIn.foliagePlacer.func_236752_a_(generationReader, rand, bruh, l1, p_236407_8_, j, l, p_225557_5_, boundingBoxIn);
                    });
                    return true;
                } else {
                    return false;
    ...
    private int func_241521_a_(IWorldGenerationBaseReader p_241521_1_, int p_241521_2_, BlockPos p_241521_3_, BaseWorldTreeFeatureConfig p_241521_4_) {
    ...
    public final boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, BaseWorldTreeFeatureConfig config) {
    ...
    private VoxelShapePart func_236403_a_(IWorld world, MutableBoundingBox bbox, Set<BlockPos> blockset1, Set<BlockPos> blockset2) {
        List<Set<BlockPos>> list = Lists.newArrayList();
        VoxelShapePart voxelshapepart = new BitSetVoxelShapePart(bbox.getXSize(), bbox.getYSize(), bbox.getZSize());
        int i = 6;
        ...
        for(BlockPos blockpos : Lists.newArrayList(blockset2)) {
            if (bbox.isVecInside(blockpos)) {
                voxelshapepart.setFilled(blockpos.getX() - bbox.minX, blockpos.getY() - bbox.minY, blockpos.getZ() - bbox.minZ, true, true);
            }
        }

        for(BlockPos blockpos1 : Lists.newArrayList(blockset1)) {
            if (bbox.isVecInside(blockpos1)) {
                voxelshapepart.setFilled(blockpos1.getX() - bbox.minX, blockpos1.getY() - bbox.minY, blockpos1.getZ() - bbox.minZ, true, true);
            }

            for(Direction direction : Direction.values()) {
                blockpos$mutable.setAndMove(blockpos1, direction);
                if (!blockset1.contains(blockpos$mutable)) {
                    BlockState blockstate = world.getBlockState(blockpos$mutable);
                    if (blockstate.hasProperty(BlockStateProperties.DISTANCE_1_7)) {
                        list.get(0).add(blockpos$mutable.toImmutable());
                        func_236408_b_(world, blockpos$mutable, blockstate.with(BlockStateProperties.DISTANCE_1_7, Integer.valueOf(1)));
                        if (bbox.isVecInside(blockpos$mutable)) {
                            voxelshapepart.setFilled(blockpos$mutable.getX() - bbox.minX, blockpos$mutable.getY() - bbox.minY, blockpos$mutable.getZ() - bbox.minZ, true, true);
                        }
                    }
                }
            }
        ...
        for(int l = 1; l < 6; ++l) {
            ...
            for(BlockPos blockpos2 : set) {
                if (bbox.isVecInside(blockpos2)) {
                    voxelshapepart.setFilled(blockpos2.getX() - bbox.minX, blockpos2.getY() - bbox.minY, blockpos2.getZ() - bbox.minZ, true, true);
                }
