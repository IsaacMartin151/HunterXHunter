...
@OnlyIn(Dist.CLIENT)
public class ManipulatorOverlay implements AutoCloseable {
    private static final ResourceLocation TEXTURE_MAP_ICONS = new ResourceLocation(HunterXHunter.MOD_ID, "textures/manipulator.png");
    private static final RenderType field_228085_d_ = RenderType.getText(TEXTURE_MAP_ICONS);
    private final TextureManager textureManager;
    private final Map<String, ManipulatorOverlay.Instance> loadedMaps = Maps.newHashMap();

    public ManipulatorOverlay(TextureManager textureManagerIn) {
    ...
    /**
     * Returns {@link com.chubbychump.hunterxhunter.client.gui.ManipulatorOverlay.Instance MapItemRenderer.Instance} with given map data
     */
    private ManipulatorOverlay.Instance getMapRendererInstance(MapData mapdataIn) {
        ManipulatorOverlay.Instance mapitemrenderer$instance = this.loadedMaps.get(mapdataIn.getName());
        if (mapitemrenderer$instance == null) {
            mapitemrenderer$instance = new ManipulatorOverlay.Instance(mapdataIn);
    ...
    @Nullable
    public ManipulatorOverlay.Instance getMapInstanceIfExists(String p_191205_1_) {
    ...
    class Instance implements AutoCloseable {
        ...
        private Instance(MapData mapdataIn) {
            ...
            ResourceLocation resourcelocation = ManipulatorOverlay.this.textureManager.getDynamicTextureLocation("map/" + mapdataIn.getName(), this.mapTexture);
            this.field_228088_d_ = RenderType.getText(resourcelocation);
        }

        /**
         * Updates a map {@link com.chubbychump.hunterxhunter.client.gui.ManipulatorOverlay.Instance#mapTexture texture}
         */
        ...
        private void func_228089_a_(MatrixStack p_228089_1_, IRenderTypeBuffer p_228089_2_, boolean p_228089_3_, int p_228089_4_) {
            ...
            for(MapDecoration mapdecoration : this.mapData.mapDecorations.values()) {
                if (!p_228089_3_ || mapdecoration.renderOnFrame()) {
                    ...
                    float f5 = -0.001F;
                    IVertexBuilder ivertexbuilder1 = p_228089_2_.getBuffer(ManipulatorOverlay.field_228085_d_);
