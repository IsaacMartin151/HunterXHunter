...
import static com.chubbychump.hunterxhunter.util.RegistryHandler.GREED_ISLAND_PORTAL;
...
public class TFTeleporter implements ITeleporter {
    ...
    public PortalInfo placeInExistingPortal(ServerWorld world, Entity entity, BlockPos pos) {
        ...
        if (!(entity instanceof PlayerEntity) && this.columnMap.containsKey(columnpos)) {
                ...
                for(int eX = -128; eX <= 128; ++eX) {
                    ...
                    for(int eZ = -128; eZ <= 128; ++eZ) {
                        for(BlockPos blockpos1 = pos.add(eX, world.getHeight() - 1 - pos.getY(), eZ); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
                            ...
                            if (world.getBlockState(blockpos1).getBlock() == GREED_ISLAND_PORTAL.get()) {
                                for(blockpos2 = blockpos1.down(); world.getBlockState(blockpos2).getBlock() == GREED_ISLAND_PORTAL.get(); blockpos2 = blockpos2.down()) {
                                    blockpos1 = blockpos2;
                                }
            ...
            if (blockpos == null) {
                ...
                if (isFrame) {
                    this.destinationCoordinateCache.put(columnpos, new TFTeleporter.PortalPosition(blockpos, world.getGameTime()));
    ...
    public void makePortal(ServerWorld world, Entity entity) {
        ...
        BlockState portal = GREED_ISLAND_PORTAL.get().getDefaultState().with(PortalBlock.AXIS, xAxis == 0 ? Direction.Axis.Z : Direction.Axis.X);
